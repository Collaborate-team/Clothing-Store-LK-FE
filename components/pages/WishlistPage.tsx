...
const WishlistPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#fcfbf7] pt-32 pb-20" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {items.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 sm:gap-16">
            {items.map((item) => (
              <div key={item.id} className="group animate-fade-in bg-[#111] border border-white/5 p-4 sm:p-6 transition-all duration-500 hover:border-white/20">
                <div className="relative aspect-[3/4] overflow-hidden border border-white/5 bg-black mb-8">
                  <Image 
                    src={item.image} 
                    alt={item.name} 
                    fill 
                  ...
                  <button 
                    ...
                    className="absolute top-4 right-4 p-3 bg-black/60 backdrop-blur-md rounded-full text-white hover:bg-white hover:text-black transition-all shadow-sm z-10 cursor-pointer opacity-0 group-hover:opacity-100 duration-500"
                  >
                    <Trash2 size={16} />
                  </button>
                  ...
                  {item.status === 'Out of Stock' && (
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex items-center justify-center">
                      <span className="px-6 py-3 border border-white/20 bg-black/60 text-white text-[9px] tracking-[0.3em] font-bold uppercase">Sold Out</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      <h3 className="text-[12px] tracking-[0.2em] font-bold uppercase text-[#fcfbf7] group-hover:text-[#c8b99a] transition-colors">{item.name}</h3>
          ...
          <div className="py-32 text-center flex flex-col items-center gap-10 bg-[#111] border border-white/5 shadow-2xl">
            <div className="w-24 h-24 rounded-full bg-black border border-white/10 flex items-center justify-center text-white/5">
        ...
        <div className="mt-40 pt-24 border-t border-white/5">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 space-y-4 md:space-y-0 text-center md:text-left">
              <div className="space-y-2 mx-auto md:mx-0">
                ...
                <h2 className="text-4xl font-light tracking-tight text-[#fcfbf7]" style={{ fontFamily: "var(--font-cormorant), 'Cormorant Garamond', serif" }}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="aspect-[3/4] bg-[#111] border border-white/5 relative group overflow-hidden">
                   <div className="absolute inset-0 bg-black group-hover:scale-105 transition-transform duration-[2s] opacity-20" />
                </div>
              ))}
            </div>
        </div>
